#!/usr/bin/env python3
"""Compare joint patch validation settings on the simulation benchmark."""
...
BASE = (
    ...
    "--chain-extension --min-copy-support-anchors 1 --min-shared-tmus 1"
)
BASE_ID85 = (
    "--max-chain-gap 500 --chain-max-gap-ratio 1.05 "
    "--min-chain-anchors 20 --min-chain-both-anchors 1 "
    "--max-patch-gap-bp 70000 --patch-window-bp 300 "
    "--min-patch-identity 0.85 --max-bundle-align-bp 50000 "
    "--chain-extension --min-copy-support-anchors 1 --min-shared-tmus 1"
)

CONDITIONS = [
    ("mst1_nojoint", BASE + " --no-joint-patch-align"),
    ("mst1_joint20k_f500", BASE + " --joint-patch-align --max-joint-patch-bp 20000 --joint-patch-flank-bp 500"),
    ("mst1_joint70k_f500", BASE + " --joint-patch-align --max-joint-patch-bp 70000 --joint-patch-flank-bp 500"),
    ("mst1_joint70k_f200", BASE + " --joint-patch-align --max-joint-patch-bp 70000 --joint-patch-flank-bp 200"),
    ("mst1_joint80k_f500", BASE + " --joint-patch-align --max-joint-patch-bp 80000 --joint-patch-flank-bp 500"),
    ("mst1_joint80k_f1000", BASE + " --joint-patch-align --max-joint-patch-bp 80000 --joint-patch-flank-bp 1000"),
    ("mst1_id85_joint80k_f500", BASE_ID85 + " --joint-patch-align --max-joint-patch-bp 80000 --joint-patch-flank-bp 500"),
]
...
def main():
    out_root = ROOT / "results/joint_patch_sweep_20260707"
    ...
    with open(summary, "a", newline="") as handle:
        ...
        for idx, (name, extra) in enumerate(CONDITIONS):
            ...
            run_name = f"vs2_jointpatch_{name}"
