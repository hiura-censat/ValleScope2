...
void write_metadata(const std::filesystem::path& path,
    ...
    output << "],\n"
           ...
           << "  \"patch_window_slack_bp\": "
           << parameters.patch_window_slack_bp << ",\n"
           << "  \"max_patch_indel_bp\": "
           << parameters.max_patch_indel_bp << ",\n"
           << "  \"min_patch_identity\": " << parameters.min_patch_identity << ",\n"
           << "  \"joint_patch_align\": "
           << (parameters.joint_patch_align ? "true" : "false") << ",\n"
           << "  \"joint_patch_flank_bp\": "
           << parameters.joint_patch_flank_bp << ",\n"
           << "  \"max_joint_patch_bp\": "
           << parameters.max_joint_patch_bp << ",\n"
